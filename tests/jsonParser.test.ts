...
import { JsonParser } from '../src/util/parsers/jsonParser';
...
describe('JSON Parser', () => {
    ...
    it('should parse JSON file correctly for book orders', async () => {
        ...
        const result = await JsonParser.parseJsonFile(bookOrders);
    ...
    it('should parse JSON file correctly for pet orders', async () => {
        ...
        const result = await JsonParser.parseJsonFile(petOrders);
    ...
    it('should throw an error if the JSON file cannot be read', async () => {
        ...
        await expect(JsonParser.parseJsonFile(bookOrders)).rejects.toThrow('Error parsing JSON file at path "src/data/book orders.json": Error reading file at path "src/data/book orders.json": File not found');
    });

    it('should throw an error if the JSON data is invalid', async () => {
        (fs.readFile as jest.Mock).mockResolvedValue('Invalid JSON');

        await expect(JsonParser.parseJsonFile(bookOrders)).rejects.toThrow('Error parsing JSON file at path "src/data/book orders.json": Error parsing JSON data: Unexpected token \'I\', "Invalid JSON" is not valid JSON');
    });
});
